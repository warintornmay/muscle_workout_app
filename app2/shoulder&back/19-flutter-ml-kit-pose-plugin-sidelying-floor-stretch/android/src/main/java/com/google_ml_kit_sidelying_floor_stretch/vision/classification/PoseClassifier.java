...
import static com.google_ml_kit_sidelying_floor_stretch.vision.classification.PoseEmbedding.getPoseEmbedding;
...
public class PoseClassifier {
   ...
   * outlier-filtering by maxDistanceTopK and meanDistanceTopK, this range is the
   * minimum of two.
   */
  ...
  public ClassificationResult classify(List<PointF3D> landmarks) {
    ...
    // * First we pick top-K samples by MAX distance. It allows to remove samples
    // that are almost
    // the same as given pose, but maybe has few joints bent in the other direction.
    // * Then we pick top-K samples by MEAN distance. After outliers are removed, we
    // pick samples
