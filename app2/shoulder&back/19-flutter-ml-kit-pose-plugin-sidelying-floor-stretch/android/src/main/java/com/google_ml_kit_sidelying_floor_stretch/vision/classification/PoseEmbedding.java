...
import static com.google_ml_kit_sidelying_floor_stretch.vision.classification.Utils.*;
...
public class PoseEmbedding {
  // Multiplier to apply to the torso to get minimal body size. Picked this by
  // experimentation.
  ...
  private static float getPoseSize(List<PointF3D> landmarks) {
    // Note: This approach uses only 2D landmarks to compute pose size as using Z
    // wasn't helpful
    ...
    // torsoSize * TORSO_MULTIPLIER is the floor we want based on experimentation
    // but actual size
    // can be bigger for a given pose depending on extension of limbs etc so we
    // calculate that.
  ...
  private static List<PointF3D> getEmbedding(List<PointF3D> lm) {
    ...
    // We use several pairwise 3D distances to form pose embedding. These were
    // selected
    // based on experimentation for best results with our default pose classes as
    // captued in the
    // pose samples csv. Feel free to play with this and add or remove for your
    // use-cases.
