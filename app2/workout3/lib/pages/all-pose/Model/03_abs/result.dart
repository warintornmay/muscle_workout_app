...
Widget textPosture(BuildContext context, int level) {
  ...
  return StreamBuilder<DocumentSnapshot>(
    ...
    builder: (context, snapshot) {
      ...
      var abdominal = postData['abdominal_crunch$levelName'] ?? '';
      var legs_raise = postData['legs_raise$levelName'] ?? '';
      var plank = postData['plank$levelName'] ?? '';
      var mountain = postData['mountain_climber$levelName'] ?? '';
      var sit_up = postData['sit_up$levelName'] ?? '';
      ...
      return Column(
        children: [
          ...
          Text(
            "Abdominal crunch: " + abdominal.toString() + ' ครั้ง',
            style: TextStyle(
                fontSize: 20, color: Colors.black, fontWeight: FontWeight.w500),
          ),
          Text(
            "Legs raise: " + legs_raise.toString() + ' ครั้ง',
            style: TextStyle(
                fontSize: 20, color: Colors.black, fontWeight: FontWeight.w500),
          ),
          Text(
            "Plank: " + plank.toString() + ' วินาที',
            style: TextStyle(
                fontSize: 20, color: Colors.black, fontWeight: FontWeight.w500),
          ),
          Text(
            "Mountain climber: " + mountain.toString() + ' ครั้ง',
            style: TextStyle(
                fontSize: 20, color: Colors.black, fontWeight: FontWeight.w500),
          ),
          Text(
            "Sit up: " + sit_up.toString() + ' ครั้ง',
            style: TextStyle(
                fontSize: 20, color: Colors.black, fontWeight: FontWeight.w500),
          ),
        ],
