...
class _PoseDetectorViewState extends State<detector_exercises_plank> {
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (context) => counter_cam(
          level: widget.level,
          routePoseName: 'detector_exercises_mountain',
        ),
      ),
    );
  }
          ...
          await users.doc(uid).get().then((DocumentSnapshot documentSnapshot) {
            if (documentSnapshot.exists) {
              ...
              if ((documentSnapshot.data()
                      as Map<String, dynamic>)['plank$level'] ==
                  null) {
                users
                    .doc(uid)
                    .set({'plank$level': _settime}, SetOptions(merge: true));
              } else {
                users.doc(uid).update({'plank$level': _settime});
              }
            } else {
