...
class detector_exercises_mountain extends StatefulWidget {
  //เปลี่ยน  detector_exercises
  final bool useClassifier;
  final bool isActivity;
  final int setStep;
  final int level; //ส่วนของ level
  const detector_exercises_mountain({
    //เปลี่ยน  detector_exercises
...
class _PoseDetectorViewState extends State<detector_exercises_mountain> {
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (context) => counter_cam(
          level: widget.level,
          routePoseName: 'detector_exercises_situp',
        ),
      ),
    );
  }
            ...
            if (documentSnapshot.exists) {
              ...
              if (poseReps != 0) {
                if ((documentSnapshot.data()
                        as Map<String, dynamic>)['mountain_climber$level'] ==
                    null) {
                  users.doc(uid).set({'mountain_climber$level': poseReps},
                      SetOptions(merge: true));
                } else {
                  users.doc(uid).update({'mountain_climber$level': poseReps});
                }
              }
            } else {
