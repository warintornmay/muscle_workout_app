...
import 'package:workout/pages/all-pose/Model/05_legs/result.dart';
import 'package:intl/intl.dart';

class detector_exercises_quad_stretch extends StatefulWidget {
  final bool useClassifier;
  final bool isActivity;
  final int level;

  const detector_exercises_quad_stretch({
...
class _PoseDetectorViewState extends State<detector_exercises_quad_stretch> {
          ...
          await users.doc(uid).get().then((DocumentSnapshot documentSnapshot) {
            if (documentSnapshot.exists) {
              ...
              if ((documentSnapshot.data() as Map<String, dynamic>)[
                      'quad_stretch_with_wall$level'] ==
                  null) {
                users.doc(uid).set({'quad_stretch_with_wall$level': _settime},
                    SetOptions(merge: true));
              } else {
                users
                    .doc(uid)
                    .update({'quad_stretch_with_wall$level': _settime});
              }
            } else {
