...
import 'package:google_ml_kit_donkey_kick/google_ml_kit.dart';
import 'package:workout/pages/all-pose/Model/count_waitcam/count_waitcam.dart';
import 'package:intl/intl.dart';

class detector_exercises_donkey_kick extends StatefulWidget {
  final bool useClassifier;
  final bool isActivity;
  final int level;

  const detector_exercises_donkey_kick({
...
class _PoseDetectorViewState extends State<detector_exercises_donkey_kick> {
                          ...
                          children: [
                            ...
                            ElevatedButton(
                              ...
                              onPressed: () {
                                ...
                                //   MaterialPageRoute(
                                //     builder: (context) =>
                                //         //     detector_exercises_quad_stretch(
                                //         //   level: widget.level,
                                //         // ),
                                //         counter_cam(
                                //       level: widget.level,
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (context) => counter_cam(
          level: widget.level,
          routePoseName: 'detector_exercises_quad_stretch',
        ),
      ),
    );
  }
            ...
            if (documentSnapshot.exists) {
              ...
              if (poseReps != 0) {
                if ((documentSnapshot.data()
                        as Map<String, dynamic>)['donkey_kick$level'] ==
                    null) {
                  users.doc(uid).set(
                      {'donkey_kick$level': poseReps}, SetOptions(merge: true));
                } else {
                  users.doc(uid).update({'donkey_kick$level': poseReps});
                }
              }
            } else {
