...
import 'package:google_ml_kit_lunches/google_ml_kit.dart';
...
class detector_exercises_lunches extends StatefulWidget {
  final bool useClassifier;
  final bool isActivity;
  final int level;

  const detector_exercises_lunches({
...
class _PoseDetectorViewState extends State<detector_exercises_lunches> {
    ...
    Navigator.of(context).pushReplacement(
      ...
      MaterialPageRoute(
        builder: (context) => counter_cam(
          level: widget.level,
          routePoseName: 'detector_exercises_squats',
        ),
      ),
    );
  }
        ...
        if (uid != null) {
          await users.doc(uid).get().then((DocumentSnapshot documentSnapshot) {
            if (documentSnapshot.exists) {
              ...
              if (poseReps != 0) {
                //ป้องกันการเก็บค่า 0 ลง database
                if ((documentSnapshot.data()
                        as Map<String, dynamic>)['lunches$level'] ==
                    null) {
                  users.doc(uid).set({'lunches$level': poseReps},
                      SetOptions(merge: true)); //เพิ่มข้อมูลลง database
                } else {
                  users.doc(uid).update({'lunches$level': poseReps});
                }
              }
            } else {
              users
                  .doc(uid)
                  .set({'lunches$level': poseReps}, SetOptions(merge: true));
