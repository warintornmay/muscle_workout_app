...
Widget textPosture(BuildContext context, int level) {
  ...
  return StreamBuilder<DocumentSnapshot>(
    ...
    builder: (context, snapshot) {
      ...
      var lunches = postData['lunches$levelName'] ?? '';
      var squats = postData['squats$levelName'] ?? '';
      var sidelying_leg_raises =
          postData['sidelying_leg_raises$levelName'] ?? '';
      var sidelying_floor_stretch =
          postData['sidelying_floor_stretch$levelName'] ?? '';
      var quad_stretch_with_wall =
          postData['quad_stretch_with_wall$levelName'] ?? '';
      ...
      return Column(
        children: [
          ...
          Text(
            "Lunches: " + lunches.toString() + ' ครั้ง',
            style: TextStyle(
                fontSize: 20, color: Colors.black, fontWeight: FontWeight.w500),
          ),
          Text(
            "Squats: " + squats.toString() + ' ครั้ง',
            style: TextStyle(
                fontSize: 20, color: Colors.black, fontWeight: FontWeight.w500),
          ),
          Text(
            "Sidelying leg raises: " +
                sidelying_leg_raises.toString() +
                ' ครั้ง',
            style: TextStyle(
                fontSize: 20, color: Colors.black, fontWeight: FontWeight.w500),
          ),
          ...
          Text(
            "Quad stretch with wall: " +
                quad_stretch_with_wall.toString() +
                ' วินาที',
            style: TextStyle(
                fontSize: 20, color: Colors.black, fontWeight: FontWeight.w500),
          ),
