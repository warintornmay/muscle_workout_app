...
import 'package:google_ml_kit_tricep_kickback/google_ml_kit.dart';
...
class detector_exercises_tricep extends StatefulWidget {
  //เปลี่ยน  detector_exercises
  final bool useClassifier;
  final bool isActivity;
  final int setStep;
  final int level; //ส่วนของ level
  const detector_exercises_tricep({
    //เปลี่ยน  detector_exercises
...
class _PoseDetectorViewState extends State<detector_exercises_tricep> {
                          ...
                          children: [
                            ...
                            ElevatedButton(
                              ...
                              onPressed: () {
                                ...
                                //   MaterialPageRoute(
                                //     builder: (context) =>
                                //         //     detector_exercises_side_arm_raises(
                                //         //   level: widget.level,
                                //         // ),
                                //         counter_cam(
                                //       level: widget.level,
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (context) => counter_cam(
          level: widget.level,
          routePoseName: 'detector_exercises_side_arm_raises',
        ),
      ),
    );
  }
            ...
            if (documentSnapshot.exists) {
              ...
              if (poseReps != 0) {
                if ((documentSnapshot.data()
                        as Map<String, dynamic>)['tricep_kickback$level'] ==
                    null) {
                  users.doc(uid).set({'tricep_kickback$level': poseReps},
                      SetOptions(merge: true));
                } else {
                  users.doc(uid).update({'tricep_kickback$level': poseReps});
                }
              }
            } else {
