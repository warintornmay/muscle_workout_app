...
import 'package:google_ml_kit_side_arm_raises/google_ml_kit.dart';
...
class detector_exercises_side_arm_raises extends StatefulWidget {
  //เปลี่ยน  detector_exercises
  final bool useClassifier;
  final bool isActivity;
  final int setStep;
  final int level; //ส่วนของ level
  const detector_exercises_side_arm_raises({
    //เปลี่ยน  detector_exercises
...
class _PoseDetectorViewState extends State<detector_exercises_side_arm_raises> {
                            ...
                            ElevatedButton(
                              ...
                              onPressed: () {
                                Navigator.push(
                                  ...
                                  MaterialPageRoute(
                                    builder: (context) =>
                                        //     detector_exercises_sidelying_floor_stretch(
                                        //   level: widget.level,
                                        // ),
                                        counter_cam(
                                      level: widget.level,
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (context) => counter_cam(
          level: widget.level,
          routePoseName: 'detector_exercises_sidelying_floor_stretch',
        ),
      ),
    );
  }
            ...
            if (documentSnapshot.exists) {
              ...
              if (poseReps != 0) {
                if ((documentSnapshot.data()
                        as Map<String, dynamic>)['side_arm_raises$level'] ==
                    null) {
                  users.doc(uid).set({'side_arm_raises$level': poseReps},
                      SetOptions(merge: true));
                } else {
                  users.doc(uid).update({'side_arm_raises$level': poseReps});
                }
              }
            } else {
