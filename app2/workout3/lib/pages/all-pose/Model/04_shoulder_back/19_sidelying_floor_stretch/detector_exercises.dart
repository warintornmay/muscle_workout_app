...
import 'package:google_ml_kit_sidelying_floor_stretch/google_ml_kit.dart';
...
class detector_exercises_sidelying_floor_stretch extends StatefulWidget {
  //เปลี่ยน  detector_exercises
  final bool useClassifier;
  final bool isActivity;
  final int setStep;
  final int level; //ส่วนของ level
  const detector_exercises_sidelying_floor_stretch({
    //เปลี่ยน  detector_exercises
...
class _PoseDetectorViewState
    extends State<detector_exercises_sidelying_floor_stretch> {
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (context) => counter_cam(
          level: widget.level,
          routePoseName: 'detector_exercises_catcow',
        ),
      ),
    );
  }
            ...
            if (documentSnapshot.exists) {
              ...
              if (poseReps != 0) {
                if ((documentSnapshot.data() as Map<String, dynamic>)[
                        'sidelying_floor_stretch$level'] ==
                    null) {
                  users.doc(uid).set(
                      {'sidelying_floor_stretch$level': poseReps},
                      SetOptions(merge: true));
                } else {
                  users
                      .doc(uid)
                      .update({'sidelying_floor_stretch$level': poseReps});
                }
              }
            } else {
