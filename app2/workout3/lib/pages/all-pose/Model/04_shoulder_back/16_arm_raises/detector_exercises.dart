...
import 'package:google_ml_kit_arm_raises/google_ml_kit.dart';
...
class detector_exercises_armraises extends StatefulWidget {
  //เปลี่ยน  detector_exercises
  final bool useClassifier;
  final bool isActivity;
  final int setStep;
  final int level; //ส่วนของ level
  const detector_exercises_armraises({
    //เปลี่ยน  detector_exercises
...
class _PoseDetectorViewState extends State<detector_exercises_armraises> {
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (context) => counter_cam(
          level: widget.level,
          routePoseName: 'detector_exercises_tricep',
        ),
      ),
    );
  }
            ...
            if (documentSnapshot.exists) {
              ...
              if (poseReps != 0) {
                if ((documentSnapshot.data()
                        as Map<String, dynamic>)['arm_raises$level'] ==
                    null) {
                  users.doc(uid).set(
                      {'arm_raises$level': poseReps}, SetOptions(merge: true));
                } else {
                  users.doc(uid).update({'arm_raises$level': poseReps});
                }
              }
            } else {
              ...
              users
                  .doc(uid)
                  .set({'arm_raises$level': poseReps}, SetOptions(merge: true));
            }
          });
        } else {
          print('No user');
