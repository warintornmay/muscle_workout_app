...
Widget textPosture(BuildContext context, int level) {
  ...
  return StreamBuilder<DocumentSnapshot>(
    ...
    builder: (context, snapshot) {
      ...
      var PushUp = postData['pushup$levelName'] ?? '';
      var KneePushUp = postData['knee_push_up$levelName'] ?? '';
      var BoxPushUp = postData['box_push_up$levelName'] ?? '';
      var ShoulderStretch = postData['shoulder_stretch$levelName'] ?? '';
      var CobraStretch = postData['cobra_stretch$levelName'] ?? '';
      ...
      return Column(
        children: [
          ...
          Text(
            "Push Up: " + PushUp.toString() + ' ครั้ง',
            style: TextStyle(
                fontSize: 20, color: Colors.black, fontWeight: FontWeight.w500),
          ),
          Text(
            "Knee Push Up: " + KneePushUp.toString() + ' ครั้ง',
            style: TextStyle(
                fontSize: 20, color: Colors.black, fontWeight: FontWeight.w500),
          ),
          Text(
            "Box Push Up: " + BoxPushUp.toString() + ' ครั้ง',
            style: TextStyle(
                fontSize: 20, color: Colors.black, fontWeight: FontWeight.w500),
          ),
          Text(
            "Shoulder Stretch: " + ShoulderStretch.toString() + ' วินาที',
            style: TextStyle(
                fontSize: 20, color: Colors.black, fontWeight: FontWeight.w500),
          ),
          Text(
            "Cobra Stretch: " + CobraStretch.toString() + ' วินาที',
            style: TextStyle(
                fontSize: 20, color: Colors.black, fontWeight: FontWeight.w500),
          ),
        ],
