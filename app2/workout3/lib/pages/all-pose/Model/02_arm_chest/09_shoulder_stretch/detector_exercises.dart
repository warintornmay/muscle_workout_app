...
import 'package:google_ml_kit_shoulder_stretch/google_ml_kit.dart';
import 'package:workout/pages/all-pose/Model/count_waitcam/count_waitcam.dart';
import 'package:intl/intl.dart';

class detector_exercises_shoulderstretch extends StatefulWidget {
  //เปลี่ยน  detector_exercises
  final bool useClassifier;
  final bool isActivity;
  final int setStep;
  final int level; //ส่วนของ level
  const detector_exercises_shoulderstretch({
    //เปลี่ยน  detector_exercises
...
class _PoseDetectorViewState extends State<detector_exercises_shoulderstretch> {
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (context) => counter_cam(
          level: widget.level,
          routePoseName: 'detector_exercises_cobra',
        ),
      ),
    );
  }
          ...
          await users.doc(uid).get().then((DocumentSnapshot documentSnapshot) {
            if (documentSnapshot.exists) {
              ...
              if ((documentSnapshot.data()
                      as Map<String, dynamic>)['shoulder_stretch$level'] ==
                  null) {
                users.doc(uid).set({'shoulder_stretch$level': _settime},
                    SetOptions(merge: true));
              } else {
                users.doc(uid).update({'shoulder_stretch$level': _settime});
              }
            } else {
