...
import 'package:google_ml_kit_box_push_up/google_ml_kit.dart';
...
class detector_exercises_boxpushup extends StatefulWidget {
  //เปลี่ยน  detector_exercises
  final bool useClassifier;
  final bool isActivity;
  final int setStep;
  final int level; //ส่วนของ level
  const detector_exercises_boxpushup({
    //เปลี่ยน  detector_exercises
...
class _PoseDetectorViewState extends State<detector_exercises_boxpushup> {
...
//===================================================
                          ...
                          children: [
                            ...
                            ElevatedButton(
                              ...
                              onPressed: () {
                                ...
                                //       level: widget.level,
                                //       routePoseName:
                                //           'detector_exercises_shoulderstretch',
                                //     ),
                                //   ),
                                // );
                                SkipPage();
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (context) => counter_cam(
          level: widget.level,
          routePoseName: 'detector_exercises_shoulderstretch',
        ),
      ),
    );
  }
            ...
            if (documentSnapshot.exists) {
              ...
              if (poseReps != 0) {
                if ((documentSnapshot.data()
                        as Map<String, dynamic>)['box_push_up$level'] ==
                    null) {
                  users.doc(uid).set(
                      {'box_push_up$level': poseReps}, SetOptions(merge: true));
                } else {
                  users.doc(uid).update({'box_push_up$level': poseReps});
                }
              }
            } else {
  ...
  Future<void> processImage(
    ...
    if (useClassifier) {
      ...
      //       await users
      //           .doc(uid)
      //           .set({'box_push_up$level': poseReps}); //เปลี่ยนชื่อ
      //     } else {
      //       print('No user');
      //     }
      //   });
